...
defaultcolortable = [(11, None), (12, None), (13, None), (7, None), (8, None), (9, None), (10, None), (3, None),
                     (4, None), (6, None), (14, None), (15, None)]
    ...
    for i in range(len(ctable) - 1, -1, -1):
        ...
        if pcolor != None and c == pcolor:  # if we found this nick's permcolor
            ...
            if pcolor != None and c != pcolor:  # if this nick has a color in the table different from its permacolor
                ...
                ctable.pop(i)
                dmsg(
                    nick + "'s permacolor was found to be different from the one in the table, and was reassigned from " + str(
                        c) + " to " + str(pcolor), "GETCOLOR")
                c = color = pcolor
                ctable.append((c, nick))
                break
            else:
...
def setcolor_command(word, word_eol, userdata):
    ...
    if paramcount == 1:  # just the nick was supplied
        ...
        if 0 <= color <= 15:
            ...
            omsg(
                "Not a valid colour! Please pick one between 0 and 15. See the 'Preferences...' for the list of colours.",
                "ERROR")
...
def message_callback(word, word_eol, userdata, attributes):
    ...
    if nicenicks_enabled:
        regex = re.compile('<(?:\"[^\"]*\"[\'\"]*|\'[^\']*\'[\'\"]*|[^\'\">])+>')
        dmsg(word, "PRINTEVENT")
        dmsg("COLORTABLE length = %d" % len(chancolortable), "PRINTEVENT")
        event_name = userdata
        nicks = []
        nick = word[0]
        nick = hexchat.strip(nick, -1, 1)  # remove existing colours
        if re.search(regex, word[1]) is not None:
            nicks = re.findall(regex, word[1])
            dmsg(nicks, "PRINTEVENT")
        ...
        if not chancolortable.has_key((net, chan)):
            ...
            dmsg("Found COLORTABLE of length " + str(
